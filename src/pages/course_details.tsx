...
const CourseDetailsPage = () => {
  ...
  useEffect(() => {
    ...
    const checkEnrollment = async () => {
      ...
      try {
        const data = await apiFetchJson<any>(`/my/enrollments/${slug}`, {
          method: 'GET',
          auth: true,
          token,
  ...
  useEffect(() => {
    ...
    const fetchProgram = async () => {
      ...
      try {
        ...
        const data = await apiFetchJson<any>(`/programs/${slug}`, {
          method: 'GET',
          locale,
          signal: controller.signal
        })
  ...
  const infoItems = useMemo(() => {
    ...
    const duration = toDisplayString(program.duration)
    const deliveryMode = toDisplayString(program.delivery_mode)
    const assessmentMethod = toDisplayString(program.assessment_method)
    const currency = toDisplayString(program.currency)
    const fees = toDisplayString(program.fees)

    return [
      ['Category', toDisplayString(program.category?.name) || 'Program'],
      ['Duration', duration || 'Not specified'],
      ['Delivery', deliveryMode || 'Not specified'],
      ['Assessment', assessmentMethod || 'Not specified'],
      ['Price', currency || fees ? `${currency} ${fees}`.trim() : 'Not specified'],
  ...
  const handlePurchase = async () => {
    ...
    try {
      const data = await apiFetchJson<any>(`/programs/${program.slug}/purchase`, {
        method: 'POST',
        auth: true,
        token
      })
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    {toDisplayString(program.overview) || 'Overview will be available soon.'}
                  </p>
                </div>
                ...
                <div>
                  ...
                  >
                    {toDisplayString(program.outcomes) || 'Learning outcomes will be available soon.'}
                  </p>
                </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ color: '#555', lineHeight: 1.75, fontSize: '14px' }}>
                        {toDisplayString(program.target_audience) || 'Not specified'}
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div style={{ color: '#555', lineHeight: 1.75, fontSize: '14px' }}>
                        {toDisplayString(program.entry_requirements) || 'Not specified'}
                      </div>
                    </div>
                  </div>
                )}
                ...
                {program.intro_text && (
                  ...
                  >
                    {toDisplayString(program.intro_text)}
                  </p>
                </div>
              )}
              </div>
