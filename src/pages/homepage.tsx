...
import { apiFetchJson, getPreferredLocale, toDisplayString } from '../services/api'
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        const [briefData, aboutData] = await Promise.all([
          apiFetchJson<{ content?: unknown }>('/pages/homepage-brief', { method: 'GET', locale }),
          apiFetchJson<{ content?: unknown }>('/pages/about-us', { method: 'GET', locale })
        ])

        setBrief(toDisplayString(briefData.content, locale))
        setAboutUs(toDisplayString(aboutData.content, locale))
      } catch (err) {
          ...
          >
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '14px', marginTop: '28px', flexWrap: 'wrap' }}>
                <button
                  type="button"
                  onClick={() => navigate(currentUser ? appRoutes.courses : appRoutes.signup)}
                  style={{
                    padding: '14px 26px',
                    border: 'none',
                    borderRadius: '999px',
