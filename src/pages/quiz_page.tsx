...
const QuizPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchQuiz = async () => {
      ...
      try {
        ...
        const statusData = await apiFetchJson<{ data?: FinalQuizStatus; message?: string }>(
          `/programs/${slug}/final-quiz`,
          {
            method: 'GET',
            locale,
            auth: true,
        ...
        if (!status?.available) {
          ...
          if (status?.already_passed) {
            setErrorMessage('You already passed the final quiz for this course.')
          } else if (status?.all_lessons_complete === false) {
            setErrorMessage('Final quiz is not available yet. Complete all lessons first.')
          } else {
            setErrorMessage(statusData?.message || 'Final quiz is not available right now.')
          }
          return
        }

        const data = await apiFetchJson<{ data?: QuizData; message?: string }>(
          `/programs/${slug}/final-quiz/start`,
          {
            method: 'POST',
            locale,
            auth: true,
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      const data = await apiFetchJson<any>(`/programs/${slug}/final-quiz/submit`, {
        auth: true,
        token,
        json: {
    ...
    >
      ...
      >
        <button
          ...
          onClick={() => {
            if (slug) {
              navigate(appRoutes.courseLearning(slug))
            } else {
                ...
                {quiz.questions.map((question, index) => (
                  ...
                  >
                    <div>
                      ...
                      >
                        {toDisplayString(question.question_text)}
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gap: '12px' }}>
                      {question.options.map((option) => {
                        ...
                        return (
                          ...
                          >
                            {toDisplayString(option.option_text)}
                          </button>
                        )
                      })}
                    </div>
