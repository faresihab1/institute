...
import { appRoutes, normalizeRedirectTarget } from '../../routes'
...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiFetchJson<any>('/login', {
        json: { email, password }
      })
      ...
      if (data) {
        ...
        navigate(normalizeRedirectTarget(searchParams.get('redirect')), { replace: true })
        return
      }
    } catch (err) {
      ...
      if (status === 401) {
        ...
        setErrorMessage((err as any)?.message || 'Please check your email and password and try again.')
