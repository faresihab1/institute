...
const VerifyPage = () => {
  ...
  const handleVerify = async (event: React.FormEvent) => {
    ...
    try {
      const data = await apiFetchJson<any>('/otp/verify', {
        json: {
          email,
      ...
      if (status === 422) {
        ...
        if (validationErrors && typeof validationErrors === 'object') {
          ...
          setErrorMessage((error as any)?.message || data?.message || 'Invalid verification code.')
        }
      } else if (status === 404) {
        setErrorMessage((error as any)?.message || data?.message || 'Verification request not found. Please register again.')
      } else {
  ...
  const handleResend = async () => {
    ...
    try {
      const data = await apiFetchJson<any>('/otp/resend', {
        json: { email }
      })
      ...
      if (status === 422) {
        ...
        if (validationErrors && typeof validationErrors === 'object') {
          ...
          setErrorMessage((error as any)?.message || data?.message || 'Could not resend verification code.')
        }
      } else {
        setErrorMessage('Network error. Please try again.')
      }
