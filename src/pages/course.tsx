    ...
    const fetchCourseData = async () => {
      ...
      try {
        ...
        const [programData, sectionsData] = await Promise.all([
          apiFetchJson<{ data?: Program; message?: string }>(`/programs/${slug}`, {
            method: 'GET',
            locale,
            auth: true,
            token,
          ...
          apiFetchJson<{ data?: Section[]; message?: string }>(`/programs/${slug}/sections`, {
            method: 'GET',
            locale,
            auth: true,
            token,
  ...
  useEffect(() => {
    ...
    const fetchSectionDetails = async () => {
      ...
      try {
        ...
        const data = await apiFetchJson<{ data: Section; message?: string }>(
          `/programs/${slug}/sections/${selectedSectionId}`,
          {
            method: 'GET',
            locale,
            auth: true,
        ...
        setSectionDetails((prev) => ({ ...prev, [selectedSectionId]: data.data }))
      } catch (error) {
        if ((error as Error).name === 'AbortError') return
  ...
  return (
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={() => navigate(slug ? appRoutes.courseDetails(slug) : appRoutes.courses)}
          style={{
            width: 'fit-content',
            padding: '10px 16px',
            borderRadius: '999px',
                ...
                {activeSection ? (
                  <>
                    <div>
                      ...
                      >
                        {toDisplayString(activeSection.title)}
                      </h2>
                      <p
                        style={{
                          margin: '14px 0 0',
                      ...
                      >
                        {toDisplayString(activeSection.description) || 'Section details will be available soon.'}
                      </p>
                    </div>
                          ...
                          >
                            <div>
                              ...
                              <div style={{ color: premiumBlue, fontWeight: 800, fontSize: '18px', lineHeight: 1.4 }}>
                                {toDisplayString(lesson.title)}
                              </div>
                              <div style={{ color: '#6b7280', marginTop: '8px', fontSize: '14px' }}>
                                {toDisplayString(lesson.type)}
                            ...
                            <button
                              ...
                              onClick={() =>
                                slug
                                  ? navigate(appRoutes.lesson(slug, lesson.id, activeSection.id))
                                  : undefined
                              }
                              style={{
                                alignSelf: 'center',
                    ...
                    {activeSection.has_quiz && (
                      ...
                      >
                        ...
                        <div style={{ fontSize: '24px', fontWeight: 800, lineHeight: 1.2 }}>
                          Complete all lessons, then take the final quiz.
                        </div>
                        <button
                          type="button"
                          onClick={() =>
                            slug
                              ? navigate(appRoutes.quiz(slug, activeSection.id))
                              : undefined
                          }
                          style={{
                            width: 'fit-content',
